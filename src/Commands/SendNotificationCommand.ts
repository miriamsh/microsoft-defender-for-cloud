...
import { extensionPrefix, smsNotificationSettings } from "../constants";
...
export async function sendSmsNotification(subscription: ISubscriptionContext, notify: Notification, alert: Alert) {
    try {
        ...
        if (ans) {
            const phoneList = getConfigurationSettings(extensionPrefix, smsNotificationSettings)[subscription.subscriptionId].to;
        ...
        const smsData = await getConfigurationSettings(extensionPrefix, smsNotificationSettings);
